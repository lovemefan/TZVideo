...
public class SignatureHelper {
    ...
    /**
     * 根据uri与当前时间戳 返回加密参数
     * @param uri url 中的 uri
     * @param requestMethod 请求方式 ，如 GET， POST
     * @param token 登录状态下可以传入token
     * @return
     */
    public static Map<String, String> getVerifyMap(String uri, String requestMethod, String token) {
        ...
        String key = "bf7dddc7c9cfe6f7";
        if (TextUtils.isEmpty(key)) {
            return null;
        }
        StringBuilder sb = new StringBuilder();
        sb.append(requestMethod);
        String encodedPath = encodedPath(uri);
...
//        sb.append(currentTimeMillis);
        sb.append(currentTimeMillis);
        try {
            map.put("_sig", URLEncoder.encode(HMACHash1.a(key, sb.toString()),"utf-8"));
        } catch (Exception e) {
            e.printStackTrace();
        }
